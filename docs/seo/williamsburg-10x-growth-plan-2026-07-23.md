...
- [`evidence/williamsburg-10x-baseline-2026-07-23.json`](./evidence/williamsburg-10x-baseline-2026-07-23.json)
- [`evidence/williamsburg-dataforseo-serp-2026-07-23.json`](./evidence/williamsburg-dataforseo-serp-2026-07-23.json)
...
- Production GA4 collection points to a stream the authenticated operating account cannot read. Repository history shows the current stream was selected intentionally in May 2023. Preserve that history: recover access rather than silently replacing the stream. A conversion baseline cannot be claimed until access is reconciled.
- Two probed paths return 404, but neither appears in the current 28-day GSC page export or repository path search. No traffic-preservation or redirect claim is made without a verified historical source.
...
|---|---|---|---|
| medical spa Williamsburg | `/` | DataForSEO organic-group #1 / absolute #4 for `medical spa williamsburg`; homepage is live 200/self-canonical. GSC query-by-page export is known to contain duplicated secondary URL rows, so it is not used here to assign competing owners. | Protect current title/H1/local relevance. Improve snippet, proof, consultation path, and GBP visibility without replacing winning phrases. |
| core Williamsburg med-spa intent | `/` | DataForSEO captured the homepage at organic-group #1 / absolute #4 for `medical spa williamsburg` and group #3 / absolute #7 for `med spa williamsburg va`. | Protect the homepage. Do not create overlapping Williamsburg or “near me” owner pages. |
| Blomdahl/medical ear piercing Williamsburg | `/procedures/blomdahl-ear-piercing` | Live 200/self-canonical; 15 clicks/394 impressions, 3.81% CTR, position 9.03 in the GSC page aggregate; DataForSEO organic-group #4 / absolute #10. | Protect the live owner. Improve CTR and support it with non-competing aftercare/cost/sensitive-ear resources. |
| filler/dermal filler Williamsburg | Intended owner: `/procedures/filler` | Live 200/self-canonical with 2 clicks/267 impressions in the page aggregate. DataForSEO instead captured `/` at organic-group #11 / absolute #18 for `lip filler williamsburg va`, so ownership is split rather than settled. | Diagnose title/internal-link/snippet alignment on the existing procedure page after the current gate. Do not create a second generic Williamsburg filler page. |
| Botox Williamsburg | Intended owner: `/procedures/botox` | Live 200/self-canonical, but absent from the current page aggregate. DataForSEO captured `/` at organic-group #9 / absolute #15 for `botox williamsburg va`. | Treat as unresolved ownership. After the current gate, reconcile homepage vs procedure-page signals and strengthen one owner without creating a duplicate. |
| PRP breast lift | `/procedures/prp-breast-lift` | Live 200/self-canonical; 3 clicks/150 impressions in the page aggregate; DataForSEO organic-group #1 / absolute #4 for `prp breast lift near me`. | Protect. Expand only from provider-authored protocol, candidacy, price, sequence, and aftercare details. |
| PRP hair restoration | `/procedures/prp-hair-restoration` | Live 200/self-canonical; 1 click/298 impressions in the page aggregate; DataForSEO organic-group #1 / absolute #4 for `prp hair restoration williamsburg va`. | Protect. Improve CTR and support from evidence-based hair-loss education. |
| Newport News med spa | `/locations/newport-news-va` | Live 200/self-canonical; 2 clicks/672 impressions, 0.30% CTR, position 19.5 in the page aggregate; DataForSEO organic-group #9 / absolute #13 for `med spa newport news va`. | PR #70 remains the next controlled local-owner test after the PR #68 D7 gate. |
...
- `botox williamsburg`: 1 click, 159 impressions, position 18.8.
- `botox williamsburg va`: 1 click, 45 impressions, position 24.3.
...
- Existing page has 267 impressions and 2 clicks.
- `dermal fillers williamsburg va` is already near page one at position 10.6.
- DataForSEO captured the homepage, not the intended filler owner, at organic-group #11 / absolute #18 for `lip filler williamsburg va`.

Focused slice:

- Treat `/procedures/filler` as the intended owner, then test whether title, content, and internal-link alignment can consolidate the split signal after the current release gate.
...
This is the clearest scalable authority cluster because the live owner is a 200/self-canonical URL with 15 clicks/394 impressions in the GSC page aggregate and was captured at organic-group #4 / absolute #10.
...
- Commercial owner: `/procedures/blomdahl-ear-piercing`.
...
- PRP hair: 298 impressions at 0.34% CTR.
- Filler: 267 impressions at 0.75% CTR.
- `medical spa williamsburg`: 576 query impressions at position 3.12 with zero recorded clicks.
...
The production GA4 ownership mismatch must be resolved before establishing the conversion baseline. Do not replace or disable the intentionally selected production stream. The exact decision is: recover Viewer-or-better access for the operating BuildLeanSaaS account; if ownership cannot be recovered, document and approve a dual-measure transition before any migration. Do not sever historical continuity.
...
| PR #70 Newport News | 2026-07-26 after D7 | Merge only when the deterministic checkpoint has complete data through 2026-07-22, returns `KEEP MEASURING`, and all live owner/canonical/indexability checks pass. |
...
1. Grant the operating BuildLeanSaaS account Viewer-or-better access to the current production GA4 property, preserving the intentionally selected stream. If the owner cannot be recovered, approve a documented dual-measure transition before migration.
